...
def detail_copy(selected, opponent, venue, factor_name, selected_delta):
    ...
    if "overall form" in factor_name:
        if factor_team == selected:
            title = f"{selected}의 장기 경기력이 {'강점으로 작용합니다' if favorable else '기대만큼 강하지 않습니다'}"
            body = (
                f"최근 경기의 승점, 득점, 실점을 시간가중 방식으로 평가합니다. 가까운 경기일수록 더 크게 반영하고 "
                f"오래된 경기의 영향은 점차 낮춥니다. 그 결과 {selected}의 전반적인 경기력이 리그 중립 수준과 비교해 "
                f"{'승리 가능성을 높이는' if favorable else '승리 가능성을 낮추는'} 방향으로 작용했습니다."
            )
        else:
            title = f"{opponent}의 장기 경기력이 {selected}에게 {'도움이 됩니다' if favorable else '부담입니다'}"
            body = (
                f"{opponent}의 최근 승점, 득점, 실점을 같은 방식으로 평가했습니다. 상대의 전반적인 경기력이 리그 중립 수준보다 "
                f"{'낮게 평가되어' if favorable else '높게 평가되어'} {selected}의 승리 가능성에 "
